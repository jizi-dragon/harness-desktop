...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .run(|app, event| {
            ...
            // otherwise it's orphaned and keeps running/serving. Every quit
            // path (the app-menu ⌘Q role on macOS, the tray "退出" item)
            // funnels into ExitRequested before the event loop ends, so this
            // is the one place it needs to happen.
