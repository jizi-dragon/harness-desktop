...
//!
//! Responsibilities:
//! - resolve the system Node executable (explicit override → PATH)
//! - resolve the `dsh` `lib/bin.js` (explicit override → the user's own
//!   install: global, then npx cache → per-user managed runtime,
//!   npm-installed only as a first-use fallback)
...
//! - once running, compare the resolved copy's version against npm latest
//!   (the shell-side port of the user's `dsh-check`) and surface an
//!   update-available hint — detect only, never auto-download
...
/// Minimum Node major version dsh itself requires — mirrors the
/// "Node.js >= 22" requirement in docs/DEVELOPMENT.md. Checked at resolve
/// time so an outdated Node surfaces as a clear boot-page message instead
/// of a cryptic dsh crash/auto-restart loop with no hint at the cause.
const NODE_MIN_MAJOR: u32 = 22;
/// The npm package this whole file manages a copy of — used both by the
/// fallback `npm install` and by the post-start update check.
...
/// Result of the one-shot post-start update check, rendered in the boot page
/// footer. `Unknown` covers "not checked yet" (attach mode, offline, unreadable
/// local version) — deliberately not distinguished in the UI: a failed check
/// must look identical to no check, never like "up to date".
...
/// platforms invoke the binary directly — no shim layer there. The child
/// also runs under the rebuilt PATH (see `effective_path`) so every npm
/// resolution here agrees with what the dsh child itself resolves, instead
/// of this GUI process's possibly-stale inherited PATH.
...
/// `node --version` stdout → major version number (`"v22.12.0\r\n"` → 22).
fn node_major(version_output: &str) -> Option<u32> {
    version_output.trim().strip_prefix('v')?.split('.').next()?.parse().ok()
}

fn resolve_node() -> Result<String, String> {
    let (node, explicit) = match env_nonempty("DSH_DESKTOP_NODE") {
        Some(node) if Path::new(&node).exists() => (plain_win_path(Path::new(&node)), true),
        Some(node) => return Err(format!("DSH_DESKTOP_NODE 指向的文件不存在: {node}")),
        None => ("node".to_string(), false),
    };
    // Probed on the same rebuilt PATH the dsh child later runs under — this
    // GUI process's own inherited PATH can be stale (see `effective_path`),
    // and that would make the probe disagree with what the spawn actually
    // resolves.
    ...
    match version.as_deref().and_then(node_major) {
        Some(major) if major >= NODE_MIN_MAJOR => Ok(node),
        Some(major) => Err(format!(
            "Node.js 版本过低（当前 v{major}.x，需要 >= v{NODE_MIN_MAJOR}）：请到 https://nodejs.org 升级{}。",
            if explicit { "，或用 DSH_DESKTOP_NODE 指定新版 node.exe" } else { "" }
        )),
        None if explicit => Err(format!("无法运行 DSH_DESKTOP_NODE 指定的 node（{node}）")),
        None => Err(
...
/// Run `npm <args…>` and return its trimmed stdout, or `None` when npm is
/// missing, exits non-zero, or prints nothing. All read-only npm queries
/// funnel through here so they inherit the same shim handling and rebuilt
/// PATH as `install_runtime`'s npm install.
fn npm_query(args: &[&str]) -> Option<String> {
    let mut cmd = npm_ecosystem_command("npm", args);
...
/// `<npm root -g>/@deepseek-ai/dsh/lib/bin.js`, if the user has a global
/// install (`npm install -g @deepseek-ai/dsh`). A deliberate, persistent
/// install — hence preferred over the npx cache.
fn resolve_global_dsh() -> Option<PathBuf> {
    let root = npm_query(&["root", "-g"])?;
    let bin = Path::new(&root).join("@deepseek-ai").join("dsh").join("lib").join("bin.js");
    bin.exists().then_some(bin)
}

/// A `@deepseek-ai/dsh` copy in the npx cache — what `npx @deepseek-ai/dsh`
/// left under `<npm cache>/_npx/<hash>/node_modules/…`. The hash directory
/// names are an npm implementation detail, not a public API, so this is
/// best-effort discovery: re-run on every start (a cleaned or evicted cache
/// just falls through to the next tier), and when several hash dirs hold
/// the package, the newest `bin.js` mtime wins — the copy pulled most
/// recently is the one the user currently means.
fn resolve_npx_cached_dsh() -> Option<PathBuf> {
    let cache = npm_query(&["config", "get", "cache"])?;
    let npx_dir = Path::new(&cache).join("_npx");
    let mut best: Option<(SystemTime, PathBuf)> = None;
    for entry in fs::read_dir(&npx_dir).ok()?.flatten() {
        let bin = entry.path().join("node_modules").join("@deepseek-ai").join("dsh").join("lib").join("bin.js");
        if !bin.exists() {
            continue;
        }
        let mtime = fs::metadata(&bin).and_then(|m| m.modified()).unwrap_or(SystemTime::UNIX_EPOCH);
        let newer = match &best {
            Some((seen, _)) => mtime > *seen,
            None => true,
        };
        if newer {
            best = Some((mtime, bin));
...
/// Resolution order for the dsh `lib/bin.js`, most-explicit first:
///
/// 1. `DSH_DESKTOP_DSH_BIN` — a specific file
/// 2. `DSH_DESKTOP_RUNTIME_DIR` — a specific `node_modules` root, but only
///    when the env var is set *and* the bin is already there; a set-but-
///    missing dir falls through to the managed install below, which
///    installs into it (preserving the documented "control where the
///    managed runtime lives" contract)
/// 3. the user's own pulled copy: global install first, then the npx cache
///    — re-discovered per start, so pulling a newer copy takes effect on
///    the next start without any cache clearing
/// 4. the managed per-user runtime dir, reused from a previous managed
///    install
/// 5. first-use fallback: `npm install` into the managed runtime dir
fn resolve_bin(app: &AppHandle, server: &Shared) -> Result<String, String> {
    ...
    let managed_bin = runtime_bin_path(&rd);
    if env_nonempty("DSH_DESKTOP_RUNTIME_DIR").is_some() && managed_bin.exists() {
        push_log(server, format!("使用 DSH_DESKTOP_RUNTIME_DIR 下的 dsh: {}", managed_bin.display()));
        return Ok(plain_win_path(&managed_bin));
    }

    // Global and npx discovery run concurrently — each is one npm
    // subprocess (~0.5–1.5s on Windows) and neither depends on the other.
    let global = thread::spawn(resolve_global_dsh);
    let npx = resolve_npx_cached_dsh();
    if let Some(bin) = global.join().ok().flatten() {
        push_log(server, format!("使用全局安装的 dsh（跳过下载）: {}", bin.display()));
        return Ok(plain_win_path(&bin));
    }
    if let Some(bin) = npx {
        push_log(server, format!("使用 npx 缓存中的 dsh（跳过下载）: {}", bin.display()));
        return Ok(plain_win_path(&bin));
    }

    if managed_bin.exists() {
        push_log(server, format!("使用已安装的 dsh 运行时: {}", managed_bin.display()));
        return Ok(plain_win_path(&managed_bin));
    }
...
fn install_runtime(app: &AppHandle, server: &Shared, rd: &Path) -> Result<(), String> {
    ...
    let target = format!("{DSH_PACKAGE}@{version}");
...
fn set_running(app: &AppHandle, server: &Shared, url: &str) {
    ...
    // One-shot update check per session, only when this process spawned the
    // server (attach mode has no resolved bin to compare). Fire-and-forget:
    // the check shells out to npm (network, ~1s) and must never delay or
    // fail the boot flow that just reached "running".
    let should_check = {
        let mut s = server.lock().unwrap();
        if s.update_checked || bin.is_none() {
            false
        } else {
    ...
    if should_check {
        ...
        thread::spawn(move || run_update_check(&app2, &srv, &bin));
    }
}
...
/// The workspace directory the server spawns in:
...
/// The version of the dsh package a resolved `lib/bin.js` belongs to, read
/// from its sibling `package.json` (`<pkg>/lib/bin.js` → two ancestors up).
/// Display-only (boot page footer): the resolved copy may be the user's own
/// global/npx install, whose version has nothing to do with the managed
/// install spec. `None` when the bin isn't resolved yet or unreadable.
fn dsh_version_of_bin(bin: &Path) -> Option<String> {
    let pkg_json = bin.ancestors().nth(2)?.join("package.json");
    let value: serde_json::Value = serde_json::from_str(&fs::read_to_string(pkg_json).ok()?).ok()?;
    value.get("version")?.as_str().map(str::to_string)
}

/// Parse `major.minor.patch` with an optional `-rc.N` suffix — the scheme
/// `@deepseek-ai/dsh` actually publishes. Anything else returns `None`.
fn version_parts(v: &str) -> Option<((u64, u64, u64), Option<u64>)> {
    let (core, rc) = match v.split_once('-') {
        Some((c, r)) => (c, Some(r.strip_prefix("rc.")?.parse::<u64>().ok()?)),
        None => (v, None),
    };
...
/// Whether `latest` is strictly newer than `current`. Prerelease ordering:
/// same core version → a release outranks any `-rc.N`, and rc numbers compare
/// numerically (`rc.10` > `rc.9`). If either side doesn't parse (upstream
/// changed its scheme), fall back to plain string inequality — same rule the
/// user's `dsh-check` uses — rather than staying silent on a real update.
fn is_newer_version(latest: &str, current: &str) -> bool {
    match (version_parts(latest), version_parts(current)) {
        (Some((lc, lr)), Some((cc, cr))) => {
            if lc != cc {
                return lc > cc;
            }
            match (lr, cr) {
...
/// Compare the resolved dsh copy's version against npm latest (the
/// shell-side port of the user's PowerShell `dsh-check`: local vs
/// `npm view @deepseek-ai/dsh version`) and notify the boot page footer via
/// the `dsh-update` event. Detect-only by design: the resolved copy may be
/// the user's own global/npx install, which this shell must not rewrite —
/// updating stays a user action (pull a newer copy, restart the app).
fn run_update_check(app: &AppHandle, server: &Shared, bin: &str) {
    let Some(current) = dsh_version_of_bin(Path::new(bin)) else {
        return;
    };
    let Some(latest) = npm_query(&["view", DSH_PACKAGE, "version"]) else {
        push_log(server, "跳过 dsh 更新检查（无法访问 npm registry）".to_string());
        return;
    };
    let status = if is_newer_version(&latest, &current) {
        push_log(
            server,
            format!("dsh 有新版本：{current} → {latest}（自行拉取新版后重启应用即可生效，不会自动下载）"),
        );
        UpdateStatus::Available { current, latest }
    } else {
        UpdateStatus::Current { version: current }
    };
    {
        let mut s = server.lock().unwrap();
        s.update_status = status.clone();
    }
    let _ = app.emit("dsh-update", status);
}
...
pub fn info(app: &AppHandle, server: &Shared) -> serde_json::Value {
    ...
    let dsh_version = bin
        .as_deref()
        .map(Path::new)
        .and_then(dsh_version_of_bin)
        .unwrap_or_else(|| env_nonempty("DSH_DESKTOP_DSH_VERSION").unwrap_or_else(|| DSH_VERSION_DEFAULT.to_string()));
    serde_json::json!({
...
mod tests {
    ...
    #[test]
    fn reads_version_from_the_resolved_bins_package_json() {
        let tmp = std::env::temp_dir().join("dsh-desktop-version-probe-test");
        let pkg = tmp.join("node_modules").join("@deepseek-ai").join("dsh");
        let lib = pkg.join("lib");
        fs::create_dir_all(&lib).unwrap();
        fs::write(
            pkg.join("package.json"),
            r#"{"name":"@deepseek-ai/dsh","version":"9.9.9-rc.0"}"#,
        )
        .unwrap();
        fs::write(lib.join("bin.js"), "").unwrap();
        assert_eq!(
            dsh_version_of_bin(&lib.join("bin.js")),
            Some("9.9.9-rc.0".to_string())
        );
        assert_eq!(dsh_version_of_bin(Path::new(r"C:\nowhere\lib\bin.js")), None);
        let _ = fs::remove_dir_all(&tmp);
    }

    #[test]
    fn node_major_parses_version_probe_output() {
        assert_eq!(node_major("v22.12.0\r\n"), Some(22));
        assert_eq!(node_major("v24\n"), Some(24));
    ...
    #[test]
    fn newer_version_orders_core_then_rc() {
        // rc bump on same core
        assert!(is_newer_version("0.1.0-rc.7", "0.1.0-rc.6"));
        // numeric, not lexicographic, rc ordering
        assert!(is_newer_version("0.1.0-rc.10", "0.1.0-rc.9"));
        assert!(!is_newer_version("0.1.0-rc.9", "0.1.0-rc.10"));
        // core version bumps outrank any rc on a lower core
        assert!(is_newer_version("0.2.0-rc.1", "0.1.9"));
        assert!(!is_newer_version("0.1.9-rc.5", "0.2.0"));
        // same core: release outranks prerelease, and vice versa
        assert!(is_newer_version("0.1.0", "0.1.0-rc.7"));
        assert!(!is_newer_version("0.1.0-rc.7", "0.1.0"));
        // equal → not newer
        assert!(!is_newer_version("0.1.0-rc.6", "0.1.0-rc.6"));
        // unparseable scheme falls back to string inequality
        assert!(is_newer_version("weird-release", "0.1.0-rc.6"));
        // one side unparseable: any difference hints (prefer a false hint
        // over silently missing a real update)
        assert!(is_newer_version("0.1.0-rc.6", "0.1.0-rc.6-extra"));
    }
}
